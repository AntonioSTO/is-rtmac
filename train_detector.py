...
def train_and_evaluate(args):
    """
    Main function to train the BINARY ACTION DETECTOR.
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")
    print("--- Training Action Detector (Binary Model) ---")
    ...
    for epoch in range(args.epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            print(f"🎉 New best DETECTOR model saved to {args.save_path} with accuracy: {best_val_accuracy:.2f}%")
